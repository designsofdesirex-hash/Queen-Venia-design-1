/**
 * Queen Venia Nyx — Core JavaScript
...
(function() {
  ...
  const initAgeGate = () => {
    ...
    on(enterBtn, 'click', () => {
      ...
      try {
        ...
        localStorage.setItem('vq_cookies', JSON.stringify({
          necessary: true,
          analytics: false,
          marketing: false,
          timestamp: Date.now()
      ...
      document.cookie = `vq_cookies=1; max-age=${TTL_DAYS * 86400}; path=/; SameSite=Lax`;
      document.documentElement.classList.add('age-verified');
      gate.hidden = true;
    });
  ...
  // --------------------------------------------------------------------------
  // 2. THEME ENGINE (Pure White Theme)
  // --------------------------------------------------------------------------
  const initThemeEngine = () => {
    document.documentElement.setAttribute('data-theme', 'white');
    try {
      localStorage.removeItem('vq_theme');
    } catch (e) {}
  };
  ...
  const initCookieBanner = () => {
    ...
    // Cookie consent is permanently consolidated into the first gate
    banner.hidden = true;
  ...
  const initHeroVideoEngine = () => {
    ...
    if (containers.length === 0) return;
        ...
        // Complete pool of 6 optimized videos
    const videoPool = [
  ...
  // --------------------------------------------------------------------------
  // TOAST NOTIFICATION & BOOKING PROTOCOL HANDLER
  // --------------------------------------------------------------------------
  let toastTimer;
...
`👑 QUEEN VENIA NYX — BUCHUNG & HINGABE-PROTOKOLL
========================================
• Ergebener Alias: ${name}
• Angegebener Benutzername: ${contact}
• Gewünschter Service: ${service}
• Zahlungsstatus: Vorauszahlung bereitgestellt
• Budget-Rahmen: ${budget}
• Zahlungsplattform: ${platform}
• Beichte / Details: ${details}
----------------------------------------
* Bestätigt: Client bestätigt, dass die Vorauszahlung mit dem Benutzernamen übermittelt wird.` :
`👑 QUEEN VENIA NYX — BOOKING & DEVOTION PROTOCOL
========================================
• Devotee Alias: ${name}
...
* Verified: Client agrees that upfront payment must accompany this booking with their attached username.`;
      ...
      navigator.clipboard.writeText(briefText).then(() => {
        ...
        showToast(isDe ? 'Öffne Telegram. Blamiere dich nicht.' : 'Opening Telegram. Try not to embarrass yourself.');
      }).finally(() => {
        setTimeout(() => {
          if (successBanner) {
          ...
          btn.disabled = false;
          // Open Telegram
          window.open('https://t.me/venianyx', '_blank', 'noopener,noreferrer');
        }, 600);
      });
    });
  };
